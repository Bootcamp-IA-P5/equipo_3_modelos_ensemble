"""
Script de entrenamiento para VGG16 usando el split en
`data/processed/garbage_classification_split/{train,val,test}/<class>/`.

Uso (ejemplo):
    python models/train_vgg16.py --data_dir data/processed/garbage_classification_split \
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Entrenamiento VGG16 - Transfer learning"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
...
def build_model(num_classes, input_shape=(224, 224, 3), lr=1e-4, fine_tune_at=5):
    ...
    # Intentar cargar pesos preentrenados; fallback a weights=None si hay mismatch
    try:
        base_model = VGG16(
        ...
        print("Construyendo VGG16 sin pesos preentrenados (weights=None).")
...
def main():
    ...
    model_path = out_dir / f"vgg16_{timestamp}.h5"
    history_path = out_dir / f"vgg16_history_{timestamp}.json"
