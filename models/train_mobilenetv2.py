"""
Script de entrenamiento para MobileNetV2 usando el split en
`data/processed/garbage_classification_split/{train,val,test}/<class>/`.

Uso (ejemplo):
    python models/train_mobilenetv2.py --data_dir data/processed/garbage_classification_split \
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Entrenamiento MobileNetV2 - Transfer learning"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
...
def build_model(num_classes, input_shape=(224, 224, 3), lr=1e-4, fine_tune_at=50):
    ...
    try:
        ...
        print("Construyendo MobileNetV2 sin pesos preentrenados (weights=None).")
        base_model = MobileNetV2(
...
def main():
    ...
    model_path = out_dir / f"mobilenetv2_{timestamp}.h5"
    history_path = out_dir / f"mobilenetv2_history_{timestamp}.json"
